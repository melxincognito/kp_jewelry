...
async function getAnalytics() {
  const [totalProducts, availableProducts, soldProducts, totalSales, recentSales, unreadMessages] =
    await Promise.all([
      db.product.count(),
...
export default async function DashboardOverviewPage() {
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        <h1 className="text-xl font-light tracking-wide text-[var(--white)]">Overview</h1>
      ...
      {data.unreadMessages > 0 && (
        ...
        >
          ...
          <p className="text-sm text-[var(--white-dim)]">
            You have{" "}
            <span className="text-[var(--gold)] font-medium">{data.unreadMessages}</span>{" "}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--black-border)]">
                  ...
                  <th className="text-right px-4 py-3 text-xs text-[var(--white-dim)]/50 font-normal tracking-widest uppercase">Price</th>
              ...
              <tbody>
                {data.recentSales.map((sale) => (
                  ...
                  >
                    <td className="px-4 py-3 text-[var(--white)]">{sale.product.name}</td>
                    <td className="px-4 py-3 text-[var(--white-dim)]">
                      {sale.buyer?.name ?? sale.buyer?.email ?? (
                        <span className="text-[var(--white-dim)]/30 italic">In-person</span>
                      )}
                    </td>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {[
          ...
          >
            <span className="text-[var(--gold)] text-lg font-light">{link.icon}</span>
